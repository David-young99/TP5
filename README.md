<link rel="stylesheet" href="https://github.com/David-young99/TP5/blob/main/style.css"/>
...
<h3>Conclusiones</h3>
Uno de los puntos que se pueden destacar como primer aspecto, son los resultados del código utilizado para detectar inundaciones, destacando con un alto contraste los cuerpos de agua en general, este contraste permite observar hasta cuerpos de agua de "menor tamaño" para el satélite. Si a esto le sumamos la posibilidad de poder cambiar las fechas de las fotografías, puede generar resultados listos para análisis más complejos. Ya este resultado permite diversos análisis, por ejemplo determinar los lugares poteciales de inundaciones en la zona, y así desarrollar medidas de prevención ante un fenómeno similar a futuro, entre otros.
...
Por otra parte, el uso del filtro de Speckle, siendo este elemento un paso fundamental para la detección de las inundaciones, ya que sino se aplica, la imagen acumula mucho ruido visual (granulado) que puede dificultar su análisis, en especial si el área de estudio es a una escala menor. Por esta razón se aplica este filtro, el resultado será una imagén con píxeles más suavizados que homogeniza regiones donde había diferencia entre los píxeles cercanos, esto debido a interferencia. 

<img src="https://github.com/David-young99/TP5/blob/main/cn1.png"></img>
<p>Fuente: Resultado del código de GEE de la región de Upala visto desde SAR en el antes y después de la aplicación del filtro de Speckle</p>

Además la aplicación de un umbral para poder separar las zonas de inundación, para esto se aplicó una fórmula, para este caso en este caso específico se utilizó el siguiente código:
